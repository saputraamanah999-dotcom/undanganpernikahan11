...
export default function App() {
  ...
  // ------------------------------------------------------------
  // Each subscription fires onSnapshot callbacks that REPLACE the
  // localStorage-based `weddingData` in React state. Firestore
  // data always wins on conflict (cross-device sync fix).
  // Dev-mode console.logs let the user verify realtime sync in
  // the browser console (e.g. when admin edits the music URL on
  // a laptop, every open browser logs the new siteSettings).
  // ============================================================
  useEffect(() => {
    seedDefaultDataIfNeeded();
    ...
    const unsubscribeSiteSettings = subscribeToSiteSettings((settings) => {
      if (import.meta.env.DEV) {
        console.log('[App] Firestore siteSettings → weddingState', {
    ...
    const unsubscribeCouples = subscribeToCouples((couplesData) => {
      if (import.meta.env.DEV) {
        console.log('[App] Firestore couples → weddingState', {
          aGroom: couplesData.coupleA?.groom?.nickname,
          bGroom: couplesData.coupleB?.groom?.nickname
        });
      }
    ...
    const unsubscribeAnnouncements = subscribeToAnnouncements((list) => {
      if (import.meta.env.DEV) {
        console.log('[App] Firestore announcements →', { count: list.length });
      }
