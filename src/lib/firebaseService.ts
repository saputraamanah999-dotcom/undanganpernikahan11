...
// ------------------------------------------------------------
// PRODUCTION MODE: No dummy announcements and NO default admin
// emails are seeded. The `admins` collection starts EMPTY — the
// user must manually add their own admin email via Firebase Console
// (see download/FIREBASE-SETUP.md). This is intentional for security.
// ============================================================
export async function seedDefaultDataIfNeeded() {
  try {
    ...
    // 4. PRODUCTION: announcements collection starts EMPTY.
    //    No dummy announcements are seeded. Admin creates announcements
    //    via the Pengumuman tab as needed.

    // 5. PRODUCTION: admins collection starts EMPTY.
    //    No default admin emails are seeded. The user must manually
    //    add their own admin email via Firebase Console (see
    //    download/FIREBASE-SETUP.md for step-by-step instructions).
...
export function subscribeToSiteSettings(onUpdate: (data: any) => void) {
  return onSnapshot(doc(db, "siteSettings", "site_config"), (snapshot) => {
    if (snapshot.exists()) {
      ...
      if (import.meta.env.DEV) {
        console.log('[Firestore] siteSettings update received →', {
...
export function subscribeToCouples(onUpdate: (couples: { coupleA: any; coupleB: any }) => void) {
  ...
  return onSnapshot(collection(db, "couples"), (snapshot) => {
    ...
    if (import.meta.env.DEV) {
      console.log('[Firestore] couples update received →', {
        hasA: !!coupleA,
        hasB: !!coupleB,
        aGroom: coupleA?.groom?.nickname,
        bGroom: coupleB?.groom?.nickname
      });
    }
...
export function subscribeToGallery(onUpdate: (photos: any[]) => void) {
  ...
  return onSnapshot(q, (snapshot) => {
    ...
    if (import.meta.env.DEV) {
      console.log('[Firestore] gallery update received →', { count: photos.length });
    }
    onUpdate(photos);
  }, (err) => {
...
export function subscribeToAnnouncements(onUpdate: (announcements: any[]) => void) {
  ...
  return onSnapshot(q, (snapshot) => {
    ...
    if (import.meta.env.DEV) {
      console.log('[Firestore] announcements update received →', { count: list.length });
    }
    onUpdate(list);
  }, (err) => {
...
export function subscribeToRSVP(onUpdate: (rsvp: any[]) => void) {
  ...
  return onSnapshot(q, (snapshot) => {
    ...
    if (import.meta.env.DEV) {
      console.log('[Firestore] rsvp update received →', { count: list.length });
    }
    onUpdate(list);
  }, (err) => {
...
export function subscribeToGuestbook(onUpdate: (wishes: any[]) => void) {
  ...
  return onSnapshot(q, (snapshot) => {
    ...
    if (import.meta.env.DEV) {
      console.log('[Firestore] guestbook update received →', { count: list.length });
    }
    onUpdate(list);
  }, (err) => {
