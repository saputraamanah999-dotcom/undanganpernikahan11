...
export default function AdminPanel({ isOpen, onClose }: AdminPanelProps) {
  ...
  useEffect(() => {
    ...
    // Load editable wedding data from localStorage (App.tsx syncs it from Firestore)
    ...
    // Also listen for live updates from App.tsx (which subscribes to Firestore siteSettings)
      ...
      if (fresh) {
        try {
          ...
          setFormData((prev: any) => {
            // Don't overwrite the form if user is actively editing — only update non-form fields
            // For simplicity, we DO overwrite, since admin usually saves before switching tabs.
  ...
  // ============================================================
  // LOGIN (Google sign-in primary; email/password legacy fallback)
  // ============================================================
  ...
  // Legacy email/password login (for backward compat with seeded admin)
  const handleLegacyLogin = (e: React.FormEvent) => {
    e.preventDefault();
    setLoginError('');
    const targetEmail = 'saputraamanah999@gmail.com';
    const targetPassword = 'saputra123@';
    const emailInput = (e.currentTarget as any).email?.value || '';
    const passwordInput = (e.currentTarget as any).password?.value || '';
    if (emailInput.trim().toLowerCase() === targetEmail && passwordInput === targetPassword) {
      localStorage.setItem('wedding_admin_logged_in', 'true');
      localStorage.setItem('wedding_admin_email', targetEmail);
      window.dispatchEvent(new CustomEvent('wedding_admin_state_changed'));
      setIsAdmin(true);
    } else {
      setLoginError('Kredensial legacy salah. Gunakan tombol "Masuk dengan Google" (email Anda harus terdaftar di koleksi admins Firestore).');
    }
  };
            ...
            {!isAdmin ? (
              ...
              >
                ...
                <div className="flex items-center space-x-2 py-1">
                  <div className="flex-1 h-[1px] bg-[#C5A059]/15" />
                  <span className="text-[9px] text-gray-500 font-mono uppercase">atau legacy</span>
                  <div className="flex-1 h-[1px] bg-[#C5A059]/15" />
                </div>
                ...
                <form onSubmit={handleLegacyLogin} className="space-y-3">
                  ...
                  <button
                    type="submit"
                    className="w-full py-2.5 bg-[#0A0A0A] border border-[#C5A059]/30 hover:bg-[#C5A059]/10 text-[#C5A059] font-bold rounded-xl text-[10px] tracking-wider uppercase transition-all cursor-pointer"
                  >
                    Login Legacy
                  </button>
                </form>
                ...
                <p className="text-[9px] text-gray-600 italic text-center pt-2">
                  * Admin default yang sudah di-seed: <br />
                  <span className="font-mono text-gray-500">gedejulianto148@gmail.com</span><br />
                  <span className="font-mono text-gray-500">saputra.developer@gmail.com</span><br />
                  <span className="font-mono text-gray-500">saputraamanah999@gmail.com</span>
                </p>
              </motion.div>
            ) : (
              /* ========== LOGGED-IN DASHBOARD ========== */
